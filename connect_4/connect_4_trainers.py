        ...
        for j in range(rounds):
            ...
            for k in range(test_every):
                ...
                for i in range(num_self_play):
                    ...
                    # We want the nn to output 1 if the current player will win,
                    # and -1 if the current player will lose.
                    # Since 1 means black wins and -1 means red wins, the following logic works.
                    for board in board_list:
                        if board.current_player == 'B':
                            history.append((board, result))
                        else:
                            history.append((board, -1 * result))
                    
                    if len(history) > max_history:
                        history = history[-max_history:]

                # Randomly select mini_batch_size number of positions to train on
                if mini_batch_size > len(history):
                    mini_batch = history
                else:
